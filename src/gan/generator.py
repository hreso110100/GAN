...
class UNetDown(nn.Module):
    def __init__(self, input_size: int, output_filters: int, normalize=True):
        ...
        self.model = Sequential(
            Conv2d(input_size, output_filters, kernel_size=(5, 5), padding=2, stride=(1, 2), bias=False),
            LeakyReLU(0.2)
        )
...
class UNetUp(nn.Module):
    def __init__(self, input_size: int, output_filters: int, dropout=0.0):
        ...
        self.model = Sequential(
            Conv2d(input_size, output_filters, kernel_size=5, stride=(1,2), padding=2, bias=False),
            ReLU(inplace=True),
...
class Generator(nn.Module):
    def __init__(self, file_shape: tuple, output_filters=8, output_channels=2):
        ...
        self.last = nn.Sequential(
            Conv2d(output_filters * 2, output_channels, kernel_size=5, stride=1, padding=2),
            Sigmoid(),
        )
